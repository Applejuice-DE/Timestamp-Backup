...
class TimestampBackup:
    ...
    def start_backup(self):
        ...
        self.log_status(f"🔄 Sammle Timestamps → {filename}")
    ...
    def backup_worker(self, folder, filename):
        try:
            ...
            # EINMALIG speichern (kein incremental wegen JSON-Fehler)
            with open(filename, "w", encoding="utf-8") as f:
                json.dump(timestamps, f, indent=2, ensure_ascii=False)

            self.root.after(0, lambda n=len(timestamps): self.log_status(f"✅ FERTIG: {n:,}/{total} Dateien → {filename}"))
    ...
    def start_restore(self):
        ...
        filename = filedialog.askopenfilename(title="Backup laden", filetypes=[("JSON", "*.json")])
        if filename:
            self.is_running = True
            self.stop_event.clear()
            self.disable_buttons()
    ...
    def restore_worker(self, folder, filename):
        try:
            ...
            self.root.after(0, lambda: self.log_status(f"📂 Lade {filename}..."))
            ...
            for i, item in enumerate(data):
                ...
                if os.path.exists(filepath):
                    try:
                        os.utime(filepath, (time.time(), item["modified"]))  # Access jetzt, Modified restore
                        restored += 1
            ...
            self.root.after(0, lambda r=restored, t=total: self.log_status(f"✅ Wiederherstellung beendet: {r}/{t} Timestamps"))

        except json.JSONDecodeError as e:
            self.root.after(0, lambda: self.log_status(f"❌ JSON-Fehler: {e} - Datei prüfen!"))
        except Exception as e:
            self.root.after(0, lambda: self.log_status(f"❌ Fehler: {e}"))
        finally:
            self.root.after(0, self.progress_stop)
